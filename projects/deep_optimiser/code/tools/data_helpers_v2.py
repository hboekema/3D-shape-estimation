...
def format_offsetable_params(offsetable_params):
    ...
    if offsetable_params == "all_pose":
        ...
        #offsetable_params_indices = [index for index in range(85) if index not in not_trainable and index < 66]
        offsetable_params = [param_ids[index] for index in offsetable_params_indices]
    elif offsetable_params == "all_pose_and_shape_and_global_rotation":
        not_trainable = [0, 2]
        offsetable_params_indices = [index for index in range(85) if index not in not_trainable and index < 82]
        offsetable_params = [param_ids[index] for index in offsetable_params_indices]
...
def offset_params(X_params, params_to_offset, DISTRACTOR=np.pi, dist="uniform"):
    ...
    #print("weights_new first entry: " +str(weights_new[0]))
    #print("offset_params_int: " + str(offset_params_int))
    #print("k: " + str(k))
    #exit(1)
...
def architecture_output_array(ARCHITECTURE, data_samples, num_trainable=24):
    ...
    if ARCHITECTURE == "OptLearnerMeshNormalStaticModArchitecture":
        ...
        Y_data = [np.zeros((data_samples, 85)), np.zeros((data_samples,)), np.zeros((data_samples, 6890, 3)), np.zeros((data_samples,)), np.zeros((data_samples,)), np.zeros((data_samples, 85)), np.zeros((data_samples, 85)), np.zeros((data_samples, 85)), np.zeros((data_samples, 85)), np.zeros((data_samples, 31)), np.zeros((data_samples, 85)), np.zeros((data_samples, 31, 3)), np.zeros((data_samples, 31, 3)), np.zeros((data_samples, 31, 3))]
    elif ARCHITECTURE == "ShapeConv2DOptLearnerArchitecture":
        Y_data = [np.zeros((data_samples, 85)), np.zeros((data_samples,)), np.zeros((data_samples, 6890, 3)), np.zeros((data_samples,)), np.zeros((data_samples,)), np.zeros((data_samples, 85)), np.zeros((data_samples, 85)), np.zeros((data_samples, 85)), np.zeros((data_samples, 85)), np.zeros((data_samples, 31)), np.zeros((data_samples, 85)), np.zeros((data_samples, 31, 3)), np.zeros((data_samples, 31, 3)), np.zeros((data_samples, 31, 3)), np.zeros((data_samples, 6890, 3)), np.zeros((data_samples, 31, 3)), np.zeros((data_samples, 31, 3))]
...
def gather_input_data(data_samples, smpl, PARAMS_TO_OFFSET, POSE_OFFSET, ARCHITECTURE, param_trainable, num_test_samples=5, MODE="RODRIGUES", LOAD_DATA_DIR=None, kin_tree=[], dist="uniform"):
    ...
    X_data = [np.array(X_indices), np.array(all_X_params), np.array(all_X_pcs), np.array(all_X_silhs)]
