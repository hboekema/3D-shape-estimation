...
#from architectures.initialisers import emb_init_weights
from architectures.initialisers.simple_initialiser import emb_init_weights


def update_weights_wrapper(DISTRACTOR, num_samples, PERIOD, trainable_params, optlearner_model, gt_data=None, generator=None, offset_nt={}, pose_offset={}, dist="uniform", reset_to_zero=False):
        ...
        if generator is not None:
            ...
            for param in param_ids:
                ...
                if not reset_to_zero:
                    ...
                    if param in offset_nt.keys() and param in not_trainable:
                        if dist == "gaussian" or dist == "normal":
                            extra_offset = [ np.random.normal(loc=0.0, scale=offset_nt[param], size=(BL_SIZE, weights[i].shape[1])) for i in range(weights.shape[0])]
                        elif dist == "uniform":
                            extra_offset = [ (1-2*np.random.rand(BL_SIZE, weights[i].shape[1])) * offset_nt[param] for i in range(weights.shape[0])]
...
def setup_train_data(trainable_params, DISTRACTOR, PARAMS_TO_OFFSET, POSE_OFFSET, ARCHITECTURE, data_samples, BATCH_SIZE, train_vis_dir, num_cb_samples=5, MODE="RODRIGUES", LOAD_DATA_DIR=None, use_generator=False, RESET_PERIOD=None, groups=None, TRAIN_PERIOD=5, OFFSET_NT={}):
...
def setup_emb_initialiser(X_params, DISTRACTOR, period=None, OFFSET_NT={}, RESET_PRED_TO_ZERO=False, DIST="uniform"):
    if RESET_PRED_TO_ZERO:
        ...
        emb_initialiser = emb_init_weights(X_params, period=period, distractor=DISTRACTOR, pose_offset=OFFSET_NT, dist=DIST)
    else:
...
def setup_train_model(ARCHITECTURE, data_samples, param_trainable, emb_initialiser, INPUT_TYPE, LR, LOSSES_WEIGHTS, GROUPS, UPDATE_WEIGHT):
    ...
    optimizer = Adam(lr=LR, decay=0.0, clipvalue=1.0)
...
def train_model(setup_params, run_id):
    ...
    # Parameter setup
    X_train, Y_train, X_cb, Y_cb, silh_cb, smpl, trainable_params, param_trainable, DISTRACTOR, POSE_OFFSET, update_generator, OFFSET_NT = setup_train_data(trainable_params, DISTRACTOR, PARAMS_TO_OFFSET, POSE_OFFSET, ARCHITECTURE, data_samples, BATCH_SIZE, train_vis_dir, num_cb_samples=NUM_CB_SAMPLES, MODE=MODE, LOAD_DATA_DIR=DATA_LOAD_DIR, use_generator=USE_GENERATOR, RESET_PERIOD=RESET_PERIOD, groups=groups, TRAIN_PERIOD=TRAIN_PERIOD, OFFSET_NT=OFFSET_NT)


    """ Model set-up """

    # Set-up the embedding initialiser
    emb_initialiser = setup_emb_initialiser(X_train[1], DISTRACTOR, period=None, OFFSET_NT=OFFSET_NT, RESET_PRED_TO_ZERO=RESET_PRED_TO_ZERO, DIST=DIST)
