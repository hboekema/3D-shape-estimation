...
def emb_init_weights_np(emb_params, distractor=np.pi, dist="uniform", pose_offset={}, reset_to_zero=False):
    ...
    def emb_init_wrapper(param, offset=False):
        def emb_init(shape, dtype="float32"):
            ...
            if reset_to_zero:
                emb_params_ = np.zeros_like(emb_params_) + 1e-5
...
def architecture_inputs_and_outputs(ARCHITECTURE, param_trainable, emb_initialiser, smpl_params, input_info, faces, data_samples, INPUT_TYPE, groups=[], update_weight=1.0):
...
def construct_optlearner_model(ARCHITECTURE, param_trainable, emb_initialiser, data_samples, INPUT_TYPE, GROUPS=[], update_weight=1.0):
    smpl_params, input_info, faces = load_smpl_params()
    print("Optimiser architecture: " + str(ARCHITECTURE))

    optlearner_inputs, optlearner_outputs = architecture_inputs_and_outputs(ARCHITECTURE, param_trainable, emb_initialiser, smpl_params, input_info, faces, data_samples, INPUT_TYPE, GROUPS, update_weight)
...
def gather_optlearner_losses(INPUT_TYPE, ARCHITECTURE, LOSS_WEIGHTS=[1.0, 1.0, 1.0]):
    ...
    if ARCHITECTURE == "NewDeepConv1DOptLearnerArchitecture" or ARCHITECTURE == "GroupedConv1DOptLearnerArchitecture" or ARCHITECTURE == "ConditionalOptLearnerArchitecture" or ARCHITECTURE == "PeriodicOptLearnerArchitecture":
