...
from tools.data_helpers import format_offsetable_params, format_distractor_dict, gather_input_data, gather_cb_data, format_joint_levels, get_new_weights, save_dist_info
...
def gen_save_suffix(JOINT_LEVELS, DIST, is_train_dist=True):
    save_suffix = ""
    if is_train_dist:
        save_suffix += "_train"
    else:
...
def create_test_dir_for_model(exp_dir, model_name, save_suffix, LEARNING_RATES):
...
def setup_test_model(model, ARCHITECTURE, data_samples, param_trainable, emb_initialiser, initial_weights, INPUT_TYPE, TEST_LR, groups, update_weight):
    ...
    # Compile the model
    #optimizer = Adam(lr=TEST_LR, decay=0.0)
    optimizer = SGD(lr=TEST_LR, momentum=0.0, nesterov=False)
    optlearner_loss, optlearner_loss_weights = gather_test_optlearner_losses(INPUT_TYPE, ARCHITECTURE)
