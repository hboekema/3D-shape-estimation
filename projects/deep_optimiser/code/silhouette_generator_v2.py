...
from tools.data_helpers_v2 import architecture_output_array, gen_data
from tools.image_helpers import normalise_img, augment_image
...
class OptLearnerUpdateGenerator(keras.utils.Sequence):
    ...
    def save_data(self):
        ...
        if self.save_path is not None:
            print("Started saving npz at " + str(datetime.now()))
            sample_path = self.save_path + "cb_samples_E{}.npz".format(self.epoch)
            if self.ARCHITECTURE == "NewDeepConv1DOptLearnerArchitecture":
                mask_to_save = np.tile(self.trainable_params_mask, (len(self.cb_samples), 1))
                np.savez(sample_path, indices=self.cb_samples, params=self.params[self.cb_samples], pcs=self.pcs[self.cb_samples],\
                        trainable_params=mask_to_save)
            elif self.ARCHITECTURE == "PeriodicOptLearnerArchitecture":
                save_params_to_train = np.tile(self.params_to_train, (len(self.cb_samples), 1))
                np.savez(sample_path, indices=self.cb_samples, params=self.params[self.cb_samples], pcs=self.pcs[self.cb_samples],\
            ...
            print("Finished saving npz at " + str(datetime.now()))
    ...
    def render_silh(self, pcs, dim=[128, 128]):
        #print("Rendering in generator...")
        X_silh = []
        for pc in pcs:
            # Render the silhouette from the point cloud
            silh = Mesh(pointcloud=pc).render_silhouette(dim=dim, show=False)
            silh = normalise_img(silh, dim=dim)
            if self.augment:
                silh = augment_image(silh)
            X_silh.append(silh)
        #print("Finished rendering.")

        return np.array(X_silh)
    ...
    def __getitem__(self, index):
        ...
        X_batch_silh = self.render_silh(X_batch_pc)
        #print(X_batch_params[0])

        #print("Batch GT mean: " + str(np.mean(X_batch_params, axis=0)) + "\nBatch GT SD: " + str(np.std(X_batch_params, axis=0)))

        X_batch = [np.array(X_batch_index), np.array(X_batch_params), np.array(X_batch_pc), np.array(X_batch_silh)]
    ...
    def yield_data(self, epoch=0, render=True):
        ...
        X_silh = self.render_silh(self.pcs)
        X_data = [np.array(ordered_indices), np.array(self.params), np.array(self.pcs), np.array(X_silh)]
