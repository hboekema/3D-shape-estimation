...
class GeneratorParamErrorCallback(tf.keras.callbacks.Callback):
    def __init__(self, run_dir, generator, period=10, ARCHITECTURE=None, num_samples=5):
        ...
        self.params_mse_log = self._create_log("params_mse.txt")
        self.params_mspe_log = self._create_log("params_mspe.txt")
        self.params_angle_log = self._create_log("params_angle.txt")

        self.gt_normals_log = self._create_log("gt_normals.txt")
        self.opt_normals_log = self._create_log("opt_normals.txt")
        self.mse_normals_log = self._create_log("mse_normals.txt")
        self.network_mse_normals_log = self._create_log("network_mse_normals.txt")

        self.delta_d_hat_log = self._create_log("delta_d_hat.txt")
        self.delta_angle_log = self._create_log("delta_angle.txt")
        self.delta_d_log = self._create_log("delta_d.txt")
        self.delta_d_abs_log = self._create_log("delta_d_magnitude.txt")
