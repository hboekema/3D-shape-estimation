...
def GroupedConv1DOptLearnerArchitecture(param_trainable, init_wrapper, smpl_params, input_info, faces, emb_size=1000, input_type="3D_POINTS", groups=[]):
    ...
    diff_normals2 = Lambda(lambda x: K.tf.subtract(x[0], x[1]), name="diff_angle")([gt_normals, opt_normals])
    ...
    #false_update_loss = Multiply()([optlearner_params, delta_d_NOGRAD])     # debugging only
    false_update_loss = Lambda(lambda x: -x, name="update_loss")(false_update_loss)
    print("smpl loss shape: " + str(false_update_loss.shape))

    return [optlearner_input, gt_params, gt_pc, gt_silh], [optlearner_params, false_loss_delta_d, optlearner_pc, false_loss_pc, false_loss_delta_d_hat, false_sin_loss_delta_d_hat,  false_update_loss, delta_d, delta_d_hat, diff_normals2_mse, per_param_mse, gt_normals, opt_normals]
