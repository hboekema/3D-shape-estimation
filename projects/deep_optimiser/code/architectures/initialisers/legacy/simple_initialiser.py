...
def emb_init_weights(emb_params, period=None, distractor=np.pi, pose_offset_non_trainable={}, dist="uniform", reset_to_zero=False):
    """ Embedding weights initialiser """
    recognised_modes = ["uniform", "gaussian", "normal"]
    assert dist in recognised_modes

    def emb_init_wrapper(param, trainable=False):
        def emb_init(shape):
            """ Initializer for the embedding layer """
            init = None
            #if offset:
            #    k = distractor["param_{:02d}".format(param)]
            #else:
            #    k = pose_offset["param_{:02d}".format(param)]

            if reset_to_zero:
                init = K.constant(np.zeros(shape), dtype="float32")
            elif trainable == False and "param_{:02d}".format(param) not in pose_offset.keys():
                init = K.constant(emb_params[:, param], dtype="float32")
            elif trainable == True:
                if dist == "uniform":
                    init = K.constant(np.random.uniform(size=shape, low=-distractor["param_{:02d}".format(param)], high=distractor["param_{:02d}".format(param)], dtype="float32"))
                else:
