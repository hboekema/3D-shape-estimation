...
from architecture_helpers import custom_mod, init_emb_layers, false_loss, no_loss, cat_xent, mape, scaled_tanh, pos_scaled_tanh, scaled_sigmoid, centred_linear, get_mesh_normals, load_smpl_params, get_pc, get_sin_metric, get_angular_distance_metric, emb_init_weights, angle_between_vectors, split_and_reshape_euler_angles, rotation_loss, rot3d_from_rodrigues
...
def NewDeepConv1DOptLearnerArchitecture(param_trainable, init_wrapper, smpl_params, input_info, faces, emb_size=1000, input_type="3D_POINTS"):
    ...
    # Get trainable parameters
    trainable_params_list = sorted([int(param.replace("param_", "")) for param, trainable in param_trainable.items() if trainable])
    print("trainable_params_list: " + str(trainable_params_list))
    non_trainable_params = [param for param in range(85) if param not in trainable_params_list]
    print("non_trainable_params: " + str(non_trainable_params))
    joints_with_trainable = sorted(np.unique([int((param - (param%3))/3) for param in trainable_params_list]))
    ...
    if input_type == "ONLY_NORMALS":
        optlearner_architecture = Dense(2**9, activation="relu")(diff_normals_NOGRAD)
    ...
    #print('optlearner_architecture shape: '+str(optlearner_architecture.shape))
    delta_d_hat = Dense(85, activation="linear", name="delta_d_hat_output")(optlearner_architecture)
    ...
    delta_d_hat_trainable = Lambda(lambda x: K.tf.gather(x, trainable_params_list, axis=-1))(delta_d_hat)
    ...
    # Re-order indices
    params_ordering = non_trainable_params + trainable_params_list
    ...
    delta_d_NOGRAD_trainable = Lambda(lambda x: K.tf.gather(x, trainable_params_list, axis=-1))(delta_d_NOGRAD)
