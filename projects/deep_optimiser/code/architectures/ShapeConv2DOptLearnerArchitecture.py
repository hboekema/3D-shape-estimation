...
from Conv2DNet import Conv2DNet, Conv2DNet_deeper


def ShapeConv2DOptLearnerArchitecture(param_trainable, init_wrapper, smpl_params, input_info, faces, PARAM_2D_TRAINABLE, emb_size=1000, input_type="3D_POINTS", groups=[], DROPOUT=0.0, include_shape=True):
    ...
    pose_param_indices = np.arange(72)
    shape_param_indices = np.arange(72, 85)
    shape_params_at_zero = Lambda(lambda x: K.tf.gather(x, shape_param_indices, axis=-1) * 0.0 + 1e-5)(optlearner_params)
    print("shape_params_at_zero_shape: " + str(shape_params_at_zero.shape))
    optlearner_params_no_shape = Lambda(lambda x: K.tf.gather(x, pose_param_indices, axis=-1))(optlearner_params)
    print("optlearner_params_no_shape: " + str(optlearner_params_no_shape.shape))
    optlearner_params_no_shape = Concatenate(axis=-1)([optlearner_params_no_shape, shape_params_at_zero])
    print("optlearner_params_no_shape: " + str(optlearner_params_no_shape.shape))
    #exit(1)

    # Ground truth parameters and point cloud are inputs to the model as well
    gt_params = Input(shape=(85,), name="gt_params")
    gt_shape_params_at_zero = Lambda(lambda x: K.tf.gather(x, shape_param_indices, axis=-1) * 0.0 + 1e-5)(gt_params)
    print("gt_shape_params_at_zero_shape: " + str(gt_shape_params_at_zero.shape))
    gt_params_no_shape = Lambda(lambda x: K.tf.gather(x, pose_param_indices, axis=-1))(gt_params)
    print("gt_params_no_shape: " + str(gt_params_no_shape.shape))
    gt_params_no_shape = Concatenate(axis=-1)([gt_params_no_shape, gt_shape_params_at_zero])
    print("gt_params_no_shape: " + str(gt_params_no_shape.shape))
    gt_pc = Input(shape=(6890, 3), name="gt_pc")
    gt_silh = Input(shape=(128, 128), name="gt_silh")
    print("gt parameters shape: " + str(gt_params.shape))
    print("gt point cloud shape: " + str(gt_pc.shape))
    print("gt silhouette shape: " + str(gt_silh.shape))
    gt_silh_grayscale = Lambda(lambda x: K.tf.expand_dims(x, axis=-1))(gt_silh)
    print("gt silhouette with channel shape: " + str(gt_silh_grayscale.shape))
    #exit(1)
    ...
    optlearner_pc_shape = get_pc(optlearner_params, smpl_params, input_info, faces)  # UNCOMMENT
    optlearner_pc_no_shape = Lambda(lambda x: get_pc(x, smpl_params, input_info, faces), name="optlearner_pc_no_shape")(optlearner_params_no_shape)
    gt_pc_no_shape = Lambda(lambda x: get_pc(x, smpl_params, input_info, faces))(gt_params_no_shape)
    if include_shape:
        optlearner_pc = Lambda(lambda x: x)(optlearner_pc_shape)
    else:
        optlearner_pc = Lambda(lambda x: x)(optlearner_pc_no_shape)
    ...
    gt_normals_TRUE_shape = get_mesh_normals(gt_pc, face_array, layer_name="gt_cross_product")
    gt_normals_TRUE_no_shape = get_mesh_normals(gt_pc_no_shape, face_array, layer_name="gt_no_shape_cross_product")
    if include_shape:
        gt_normals_TRUE = gt_normals_TRUE_shape
    else:
        gt_normals_TRUE = gt_normals_TRUE_no_shape
    gt_normals_TRUE_NOGRAD = Lambda(lambda x: K.stop_gradient(x))(gt_normals_TRUE)
    #gt_normals = Conv2DNet(gt_silh_grayscale, DROPOUT, PARAM_2D_TRAINABLE)
    gt_normals = Conv2DNet_deeper(gt_silh_grayscale, DROPOUT, PARAM_2D_TRAINABLE)
    gt_normals = Lambda(lambda x: x, name="gt_normals_pred")(gt_normals)
    gt_normals_LOSS = Lambda(lambda x: K.square(x[0] - x[1]), name="gt_normals_LOSS")([gt_normals_TRUE_NOGRAD, gt_normals])
    ...
    # Calculate the cross product between the GT and predicted normals
    crossproduct_normals = Lambda(lambda x: K.tf.cross(x[0], x[1]), name="diff_cross_product")([gt_normals, opt_normals])
    #crossproduct_normals_flattened = Flatten()(crossproduct_normals)
    crossproduct_normals_NOGRAD =  Lambda(lambda x: K.stop_gradient(x))(crossproduct_normals) # This is added to avoid influencing embedding layer parameters by a "bad" gradient network

    # Calculate the difference between the GT and predicted normals
    diff_normals2 = Lambda(lambda x: K.tf.subtract(x[0], x[1]), name="diff_angle")([gt_normals_TRUE, opt_normals])
    #diff_normals2_flattened = Flatten()(diff_normals2)
    diff_normals2_NOGRAD =  Lambda(lambda x: K.stop_gradient(x))(diff_normals2)
    diff_normals2_mse = Lambda(lambda x: K.mean(K.square(x), axis=-1), name="diff_angle_mse")(diff_normals2)
    diff_normals2_mse_NOGRAD =  Lambda(lambda x: K.stop_gradient(x))(diff_normals2_mse)

    crossproduct_normals_NOGRAD = Flatten()(crossproduct_normals_NOGRAD)
    diff_normals2_NOGRAD = Flatten()(diff_normals2_NOGRAD)
    mesh_diff_NOGRAD = Concatenate()([crossproduct_normals_NOGRAD, diff_normals2_NOGRAD])
    #mesh_diff = Concatenate()([crossproduct_normals_flattened, diff_normals2_flattened])
    ...
    # Extract non-trainable parameters and set predictions to 0.0
    delta_d_hat_non_trainable_NOGRAD = Lambda(lambda x: K.stop_gradient(x * 0.0))(delta_d_hat_non_trainable)  # NEWLY ADDED NEED TO BE CAREFUL
    ...
    assert len(set(params_ordering)) == 85
    for i in range(85):
    ...
    false_update_loss = Multiply()([optlearner_params, delta_d_hat_NOGRAD])
    false_update_loss = Lambda(lambda x: -x, name="update_loss")(false_update_loss)
    print("smpl loss shape: " + str(false_update_loss.shape))

    return [optlearner_input, gt_params, gt_pc, gt_silh], [optlearner_params, false_loss_delta_d, optlearner_pc_shape, false_loss_pc, false_loss_delta_d_hat, false_sin_loss_delta_d_hat,  false_update_loss, delta_d, delta_d_hat, diff_normals2_mse, per_param_mse, gt_normals_TRUE_shape, opt_normals, gt_normals_LOSS, optlearner_pc_no_shape, gt_normals_TRUE_no_shape, gt_normals]
