...
if len(models) == 0:
    ...
    model_names = [model for model in models if (int(model[model.find(".")+1 : model.find("-")]) + 1) % 100 == 0]
    model_names.sort(key=lambda x: float(x[x.find("model.")+1 : x.find("-")]))
...
for model_num, model in enumerate(models):
    ...
    logs_dir = logs_dirs[model_num]
    test_vis_dir = test_vis_dirs[model_num]
