...
def OptLearnerStaticAngleMetricArchitecture(param_trainable, init_wrapper, emb_size=1000):
    """ Optimised learner network architecture """
    # An embedding layer is required to optimise the parameters
    #print('parameter initializer pose '+str(parameter_initializer([1000,85])[0,:15]))
    #print('parameter initializer shape '+str(parameter_initializer([1000,85])[0,72:82]))
...
#    print("point cloud loss shape: " + str(false_loss_pc.shape))
    ...
    delta_d_metric = Lambda(lambda x: K.mean(K.square(K.tf.math.sin(0.5*x)), axis=1))(delta_d_NOGRAD)
    delta_d_metric = Reshape(target_shape=(1,), name="delta_d_mse")(delta_d_metric)
    print("delta_d metric schape: " + str(delta_d_metric.shape))
    ...
    return [optlearner_input, gt_params, gt_pc], [optlearner_params, delta_d_metric, optlearner_pc, false_loss_pc, false_loss_delta_d_hat, false_loss_smpl, delta_d, delta_d_hat,delta_d_hat_NOGRAD]
