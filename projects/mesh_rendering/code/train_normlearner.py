...
from optlearner import NormLearnerArchitecture, false_loss, no_loss
from smpl_np import SMPLModel
from render_mesh import Mesh
from callbacks import OptLearnerPredOnEpochEnd
from silhouette_generator import NormLearnerDataGenerator
...
# Load the SMPL data
train_gen = NormLearnerDataGenerator(train_dir, batch_size=batch_size, debug=True)
val_gen = NormLearnerDataGenerator(val_dir, batch_size=batch_size)
test_gen = NormLearnerDataGenerator(test_dir, batch_size=batch_size)
...
embedding_initializer = train_gen.yield_params(offset)

normlearner_inputs, normlearner_outputs = NormLearnerArchitecture(embedding_initializer)
print("normlearner inputs " +str(normlearner_inputs))
print("normlearner outputs "+str(normlearner_outputs))
normlearner_model = Model(inputs=normlearner_inputs, outputs=normlearner_outputs)
normlearner_model.summary()

learning_rate = 0.001
normlearner_model.compile(optimizer=Adam(lr=learning_rate, decay=0.0001), loss=[no_loss, false_loss, no_loss, false_loss],
        loss_weights=[0.0, 1.0, 0.0, 1.0])
...
Y_batch_3 = Y_batch[2]
Y_batch_4 = Y_batch[3]
print("Y normlearner_params shape: " + str(Y_batch_1.shape))
print("Y delta_d_loss shape: " + str(Y_batch_2.shape))
print("Y normlearner_pc shape: " + str(Y_batch_3.shape))
print("Y pointcloud_loss shape: " + str(Y_batch_4.shape))

# Run the main loop
normlearner_model.fit_generator(
    generator=train_gen,
    epochs=epochs,
...
# Store the model
normlearner_model.save_weights(model_dir + "model.final.hdf5")
